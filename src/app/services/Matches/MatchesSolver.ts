  ...
  private AddMatches(equation: Equation, matchesToMove: number, avilableToPlace: number) {
    ...
    for (let index = 0; index < arrays.length; index++) {
      ...
      if (arrays[index][i] == 0) {
        ...
        if (avilableToPlace == 0) {
          if (this.helper.isValidEquationNumbers(equation) && this.helper.isValidEquationResult(equation) && !this.helper.isEquationExist(this.solution,equation))
    ...
    if (equation.sign[0] == 0) {
      ...
      equation.sign[0] = 1;
        ...
        if (avilableToPlace == 0) {
          if (this.helper.isValidEquationResult(equation) && !this.helper.isEquationExist(this.solution,equation))
            this.solution.push(equation.clone());
        }
        else {
          this.AddMatches(equation, matchesToMove,avilableToPlace);
